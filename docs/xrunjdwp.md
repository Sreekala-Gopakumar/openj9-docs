...
- You can enable the JDWP support on Checkpoint/Restore In Userspace (CRIU) restore by specifying the following command-line options pre-checkpoint:

    - `-Xrunjdwp`
    - `-agentlib:jdwp`
    - `-agentpath:/path/to/libjdwp.so`

- In OpenJ9, instead of specifying `-Xrunjdwp`, `-agentlib:jdwp`, or `-agentpath:/path/to/libjdwp.so` as command-line options pre-checkpoint, you can add these command-line options in a restore option file and specify it by using the `CRIUSupport.registerRestoreOptionsFile` API when you restore a checkpoint image.
- Although both the `-Xrunjdwp` and `-agentlib:jdwp` options are available, the `-agentlib:jdwp` option is preferable because the JDWP agent uses the Java Virtual Machine Tool Interface (JVMTI) interface to the VM rather than the older Java Virtual Machine Debug Interface (JVMDI) interface.
